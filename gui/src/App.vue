  ...
  <div id="app">
    <header>
      <nav>
        <div class="nav-wrapper">
          ...
          <a v-if="myOrigin !== ''" href="/" class="brand-logo right">connected as
            <chip :name="myOrigin"></chip>
          </a>
        </div>
      </nav>
    </header>
    ...
    <div class="row">
      <div class="col s8">
        <div class="col s6">
          <contacts
            :origins-and-badges="originsAndBadges"
        ...
        <chat v-if="selectedDest === ''"
              title="Rumors Chat"
              :my-origin="myOrigin"
              :chat-messages="rumors"
              :send-msg="sendRumor"
        />
        <chat v-else
              :dest="selectedDest"
              :my-origin="myOrigin"
              :chat-messages="selectedDestChat"
              :send-msg="sendPrivate"
        />
      </div>
      <div class="col s4">
        <simple-list
          title="Indexed & Downloaded Files"
          :list="indexedFiles"
...
<script>
  ...
  export default {
    name: 'app',
    components: {DownloadSearch, RequestFile, SimpleInput, Chip, Contacts, Chat, SimpleList},
    data() {
      return {
        apiURL: '',
        ws: null, // websocket
      ...
      axios.get('http://' + this.apiURL + '/id')
        ...
        `,
          ...
          self.ws.addEventListener('message', function (e) { // here we receive packets from the websocket
            ...
            if (packet["searched-file"]) {
              self.handleSearchedFile(packet["searched-file"]);
              return;
            }
    ...
    methods: {
      ...
      handleSearchedFile: function (searchedFile) {
        this.searchedFiles.push(searchedFile)
      },

      searchFiles: function (keywordsString) {
